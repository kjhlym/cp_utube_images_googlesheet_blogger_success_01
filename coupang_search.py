        ...
        try:
            ...
            if response and 'data' in response:
                ...
                for item in products.get('productData', []):
                    ...
                    if not all([product_name, product_price, product_url]):
                ...
                return {"data": processed_products}
            else:
...
def search_coupang(keyword, max_products=3, price_range=None):
    ...
    try:
        ...
        for item in response["data"]:
            # 가격 추출 및 변환
            price = int(re.sub(r'[^0-9]', '', str(item.get("productPrice", "0"))))
            
            # 가격 범위 필터링
            if price_range:
                if price < price_range["min"] or price > price_range["max"]:
                    print(f"   가격 범위 제외 ({price:,}원): {item.get('productName', '')}")
                    continue
            
            # 필수 필드 확인
            if not all([item.get("productName"), item.get("productUrl")]):
                print(f"   ⚠️ 필수 필드 누락으로 제외: {item.get('productName', '')}")
                continue
            ...
            print(f"   ✅ 상품 추가: {item.get('productName', '')} ({price:,}원)")
        
        # 리뷰 수와 평점으로 정렬
        products.sort(key=lambda x: (int(x.get("reviewCount", "0")), float(x.get("rating", "0"))), reverse=True)
